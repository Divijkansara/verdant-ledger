  ...
  const settings = {
      ...
      return `
        <div class="module">
          <div class="wall g2">
            ...
            <div class="panel">
              <div class="panel-hd"><h2>Ledger</h2></div>
              <div class="prose" style="font-size:var(--fs-small)">
                <p>The demo ledger is deterministic — the same twelve months are generated on every
                   machine, including one planted anomaly in August for the detector to find.</p>
              </div>
              <div class="col mt-16" style="gap:9px">
                <button class="btn" id="reseedBtn">${icon("reset",14)} Regenerate the demo ledger</button>
          ...
          <!-- ═══════════ THEME LAB ═══════════ -->
          <div class="wall stack"><div class="panel">
            <div class="panel-hd"><h2>Theme lab</h2>
              <span class="hd-note">EVERY COLOUR IS GENERATED, NOT LISTED</span></div>
            <div class="prose" style="font-size:var(--fs-small);margin-bottom:16px">
              <p>There are no hard-coded colours in this product. Move any control below and the
                 entire site — surfaces, ink, borders, chart series, semantic states — is
                 regenerated by perceptual colour maths, with every text/background pair
                 contrast-solved in code before it is applied.</p>
            </div>

            <div class="preset-grid" id="presets"></div>

            <div class="wall g2 stack" style="border:0;background:transparent;gap:16px">
              <div class="panel" style="border:1px solid var(--line-1);border-radius:var(--r-md)">
                <div class="panel-hd"><h2>Controls</h2></div>
                <div class="col" style="gap:18px">
                  <div>
                    <div class="row between"><span class="eyebrow">Mode</span></div>
                    <div class="seg mt-8" id="modeSeg">
                      <button data-mode="dark">Dark</button><button data-mode="light">Light</button>
                    </div>
                  </div>
                  <div>
                    <div class="row between center"><span class="eyebrow">Accent hue</span>
                      <span class="mono faint" id="vAccentHue"></span></div>
                    <div class="hue-track mt-8"></div>
                    <input type="range" id="accentHue" min="0" max="360" step="1">
                  </div>
                  <div>
                    <div class="row between center"><span class="eyebrow">Accent intensity</span>
                      <span class="mono faint" id="vAccentC"></span></div>
                    <input type="range" id="accentChroma" min="4" max="24" step="1">
                  </div>
                  <div>
                    <div class="row between center"><span class="eyebrow">Accent loudness</span>
                      <span class="mono faint" id="vAccentContrast"></span></div>
                    <input type="range" id="accentContrast" min="46" max="150" step="2">
                  </div>
                  <div>
                    <div class="row between center"><span class="eyebrow">Surface hue</span>
                      <span class="mono faint" id="vBaseHue"></span></div>
                    <div class="hue-track mt-8"></div>
                    <input type="range" id="baseHue" min="0" max="360" step="1">
                  </div>
                  <div>
                    <div class="row between center"><span class="eyebrow">Surface tint</span>
                      <span class="mono faint" id="vBaseC"></span></div>
                    <input type="range" id="baseChroma" min="0" max="5" step="0.2">
                  </div>
                  <div>
                    <div class="row between center"><span class="eyebrow">Ground level</span>
                      <span class="mono faint" id="vBaseLevel"></span></div>
                    <input type="range" id="baseLevel" min="0" max="12" step="0.5">
                  </div>
                  <div>
                    <div class="row between center"><span class="eyebrow">Contrast</span>
                      <span class="mono faint" id="vContrast"></span></div>
                    <input type="range" id="contrast" min="80" max="130" step="1">
                  </div>
                  <div class="row wrapflex" style="gap:9px">
                    <button class="btn btn-sm" id="randomTheme">${icon("bolt",13)} Random</button>
                    <button class="btn btn-sm" id="copyTheme">${icon("copy",13)} Copy CSS</button>
                    <button class="btn btn-sm" id="resetTheme">${icon("reset",13)} Reset</button>
                  </div>
                </div>
              </div>

              <div class="panel" style="border:1px solid var(--line-1);border-radius:var(--r-md)">
                <div class="panel-hd"><h2>Live contrast audit</h2>
                  <span class="hd-note" id="auditVerdict"></span></div>
                <div class="audit" id="auditRows"></div>
                <div class="panel-hd" style="margin-top:18px"><h2>Generated tokens</h2>
                  <span class="hd-note" id="deltaE"></span></div>
    ...
    mount() {
      ...
      $("#reseedBtn").onclick = async () => {
        const ok = await dialog({ title:"Regenerate the demo ledger?",
          body:"<p>Every entry is replaced with a freshly generated twelve months, and the chain is re-sealed. Anything you posted will be lost.</p>",
          confirmLabel:"Regenerate", danger:true });
        if (!ok) return;
        S().resetLedger();
        V.App.refreshShell();
        settings.mount();
        toast("Ledger regenerated", `${S().entries.length} entries · chain re-sealed`, "info");
      };
      ...
      const paintPresets = () => {
        $("#presets").innerHTML = T.PRESETS.map(p => {
          const { tokens } = T.generate(p.seed);
          const same = JSON.stringify(p.seed) === JSON.stringify(T.seed);
          return `<button class="preset ${same ? "on" : ""}" data-preset="${p.id}">
              <span class="p-swatch">
                <i style="background:${tokens["--bg"]}"></i>
                <i style="background:${tokens["--surface-3"]}"></i>
                <i style="background:${tokens["--accent"]}"></i>
                <i style="background:${tokens["--series-3"]}"></i>
                <i style="background:${tokens["--text-1"]}"></i>
              </span>
              <b>${p.name}</b><span>${p.note}</span>
            </button>`;
        }).join("");
        $$("#presets .preset").forEach(b => b.addEventListener("click", () => {
          T.usePreset(b.dataset.preset);
          syncControls(); paintAudit(); paintPresets();
          toast("Theme applied", b.querySelector("b").textContent, "info");
        }));
      };

      const controls = [
        ["accentHue", "accentHue", 1, v => `${v}°`],
        ["accentChroma", "accentChroma", 100, v => `${(v/100).toFixed(2)}`],
        ["accentContrast", "accentContrast", 10, v => `${(v/10).toFixed(1)}:1`],
        ["baseHue", "baseHue", 1, v => `${v}°`],
        ["baseChroma", "baseChroma", 100, v => `${(v/100).toFixed(3)}`],
        ["baseLevel", "baseLevel", 100, v => `+${(v/100).toFixed(3)}`],
        ["contrast", "contrast", 100, v => `${(v/100).toFixed(2)}×`]
      ];

      function syncControls() {
        controls.forEach(([id, key, scale, fmt]) => {
          const input = $("#" + id);
          const DEF = { accentContrast: 4.6, baseLevel: 0 };
          const raw = T.seed[key] ?? DEF[key] ?? 0;
          // Let the input snap to its own step rather than rounding to an
          // integer — baseLevel moves in 0.005 increments.
          input.value = raw * scale;
          paintRange(input);
          $("#v" + id.charAt(0).toUpperCase() + id.slice(1).replace("Hue","Hue").replace("Chroma","C"))
            && ($("#v" + id.charAt(0).toUpperCase() + id.slice(1).replace("Chroma","C")).textContent = fmt(input.value));
        });
        $("#vAccentHue").textContent = `${Math.round(T.seed.accentHue)}°`;
        $("#vAccentC").textContent = T.seed.accentChroma.toFixed(3);
        $("#vBaseHue").textContent = `${Math.round(T.seed.baseHue)}°`;
        $("#vBaseC").textContent = T.seed.baseChroma.toFixed(3);
        $("#vAccentContrast").textContent = `${(T.seed.accentContrast ?? 4.6).toFixed(1)}:1`;
        $("#vBaseLevel").textContent = `+${(T.seed.baseLevel ?? 0).toFixed(3)}`;
        $("#vContrast").textContent = `${T.seed.contrast.toFixed(2)}×`;
        $$("#modeSeg button").forEach(b =>
          b.setAttribute("aria-pressed", String(b.dataset.mode === T.seed.mode)));
      }

      function paintAudit() {
        const r = T.report || T.apply();
        $("#auditVerdict").innerHTML = r.allPass
          ? `<span style="color:var(--good)">ALL GATES PASS</span>`
          : `<span style="color:var(--bad)">${r.checks.filter(c=>!c.pass).length} FAILING</span>`;
        $("#auditRows").innerHTML = r.checks.map(c => `
          <div class="audit-row">
            <span class="a-name">${c.label}</span>
            <span class="a-ratio">${c.ratio.toFixed(2)}:1</span>
            <span class="a-pass ${c.pass ? "y" : "n"}">${c.pass ? "PASS" : "FAIL"}</span>
          </div>`).join("");
        $("#deltaE").textContent = `SERIES ΔE ${r.seriesDeltaE} · TARGET ≥ 8`;

        const show = ["--bg","--surface-1","--surface-3","--line-2","--text-1","--text-2",
          "--accent","--on-accent","--good","--warn","--bad",
          "--series-1","--series-2","--series-3","--series-4","--series-5","--series-6",
          "--series-7","--series-8"];
        $("#swatches").innerHTML = show.map(k => `
          <div class="swatch"><div class="sw-c" style="background:var(${k})"></div>
            <div class="sw-n">${k.replace("--","")}<br>${T.value(k)}</div></div>`).join("");
      }

      controls.forEach(([id, key, scale]) => {
        const input = $("#" + id);
        input.addEventListener("input", () => {
          T.set({ [key]: +input.value / scale });
          paintRange(input);
          syncControls(); paintAudit(); paintPresets();
          V.App.repaintCharts();
        });
      ...
      $("#randomTheme").onclick = () => {
        T.set({
          ...
          accentChroma: 0.10 + Math.random() * 0.10,
          accentContrast: 4.6 + Math.random() * 9,
          baseChroma: Math.random() * 0.035,
          baseLevel: Math.random() * 0.10,
          mode: Math.random() > 0.35 ? "dark" : "light"
        });
        syncControls(); paintAudit(); paintPresets();
        V.App.repaintCharts();
        const r = T.report;
        toast(r.allPass ? "Random palette — all gates pass" : "Random palette",
          `accent ${Math.round(T.seed.accentHue)}° · ${T.seed.mode}`, r.allPass ? "ok" : "err");
      };
      $("#copyTheme").onclick = async () => {
        const ok = await V.UI.copyText(T.toCSS());
        toast(ok ? "Palette copied" : "Could not copy",
          ok ? "CSS custom properties on the clipboard" : "Clipboard unavailable", ok ? "ok" : "err");
      };
      $("#resetTheme").onclick = () => {
        T.usePreset("nebula");
        syncControls(); paintAudit(); paintPresets();
      ...
      paintPresets(); syncControls(); paintAudit();
    }
  };
